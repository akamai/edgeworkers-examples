...
function getRedirectLocationFromEdge(request){
    if(request.path.startsWith('/faq.html')){
        return request.url.replace('/faq.html','/faq.php');
    }
    else if(request.path.includes('/paints/')){
        return request.url.replace('/paints/','/colors/');
    }
