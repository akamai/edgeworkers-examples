...
const UNSAFE_RESPONSE_HEADERS=['content-length', 'transfer-encoding', 'connection', 'vary','accept-encoding', 
'content-encoding', 'keep-alive','proxy-authenticate', 'proxy-authorization', 'te', 'trailers','transfer-encoding', 'upgrade'];
...
function sendRecursiveOriginRequest(scheme,host,url,enableLiquidation){
    ...
    //A redirect was previously detected and enableLiquidation turned on. The origin response body should be modified
        ...
        rewriter.onElement(INJECT_AFTER_TAG, el => {
            el.after('<script>history.replaceState("","","'+url+'");</script>');
            });
