...
declare class Watermarking {
     ...
     */
    constructor(wmOptions?: WMOptions);
    /**
     * Validates the short token as per the validation rules, checks for expiry, performs signature verfication for each key from @param keys array.
     * Validation Rules:
     * - Short Tokens are four sets of base64url encoded fields joined by a period (‘.’). The fields are, in order, the expiration, the title, the wmid, and the signature.
     * - Token should be signed using HS256 and signature should contain only first 30 bytes.
     * @param authToken Short authentication token in base64 encoded string joined by a period (‘.’).
     * @param keys      Array of hex encoded keys used for verification of authToken.
     * @throws {Error} with appropriate error message if any validation check fails.
     ...
     * @example Error(400, 'Invalid token type, expected string!') - If token type check fails.
     * @example Error(400, 'Invalid keys type, expected array of hex encoded string!') - If keys type check fails.
     * @example Error(400, 'Token malformed: invalid short token format, expected token with four sets of base64url encoded fields joined by a period ‘.’!') - If short token format is invalid.
     * @example Error(400, 'Token expired!') - If short token is expired.
     * @example Error(400, 'Short token signature verification failed!') - If signature verification failed on the token.
     */
    validateShortToken(authToken: string, keys: string[]): Promise<ShortToken>;
    /**
     ...
     */
    validateToken(authToken: Uint8Array | string, keys: string[]): Promise<WMJSON>;
    /**
     ...
     * @param authToken   JWT/CWT Auth token
     * @returns           List of CryptoKey instances
     */
    private importCryptoKeys;
}

export { CWTUtil, CWTValidator, JWTValidator, ShortToken, TokenType, VariantSubPath, WMJSON, WMOptions, WMPayload, Watermarking };
